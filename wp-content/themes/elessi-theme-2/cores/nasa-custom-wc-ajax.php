             ...
             */
                ...
                if (!$woocommerce || !isset($_REQUEST['product_id']) || !is_numeric(wp_unslash($_REQUEST['product_id']))){
                    ...
                    wc_get_template("notices/error.php", array(
                        'notices' => array(
                            array(
                                'notice' => esc_html__('Sorry, Product is not existing.', 'elessi-theme')
                            )
                        ),
                    ));
                ...
                if (!$error && $validate_attr['validate'] && $passed_validation && 'publish' === $product_status && $woocommerce->cart->add_to_cart($product_id, $quantity, $variation_id, $variation)) {
                    ...
                    if (isset($validate_attr['missing_attributes'])) {
                        $mess = array(
                            array(
                                'notice' => sprintf(_n('%s is a required field', '%s are required fields', count($validate_attr['missing_attributes']), 'elessi-theme'), wc_format_list_of_items($validate_attr['missing_attributes']))
                            )
                        );
                    } else {
                        $mess = array(
                            array(
                                'notice' => esc_html__('Sorry, Maybe product empty in stock.', 'elessi-theme')
                            )
                        );
                    }
