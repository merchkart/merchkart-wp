...
if (!function_exists('elessi_add_action_woo')) :
    function elessi_add_action_woo() {
        ...
        add_action('woocommerce_after_shop_loop_item', 'elessi_centent_show_in_list');
 ...
 */
if (!function_exists('elessi_centent_show_in_list')) :
    function elessi_centent_show_in_list($show_in_list) {
...
if (!function_exists('elessi_quickview_in_list')) :
    function elessi_quickview_in_list($echo = true) {
        ...
        $quickview = '<a href="javascript:void(0);" class="quick-view btn-link quick-view-icon tip-top" data-prod="' . absint($product->get_id()) . '" data-tip="' . ($type !== 'woosb' ? esc_attr__('Quick View', 'elessi-theme') : esc_attr__('View', 'elessi-theme')) . '" title="' . ($type !== 'woosb' ? esc_attr__('Quick View', 'elessi-theme') : esc_attr__('View', 'elessi-theme')) . '" data-product_type="' . esc_attr($type) . '" data-href="' . get_the_permalink() . '">' .
            '<i class="nasa-icon pe-7s-look"></i>' .
            '<span class="hidden-tag nasa-icon-text">' .
                ($type !== 'woosb' ? esc_html__('Quick View', 'elessi-theme') : esc_html__('View', 'elessi-theme')) .
            '</span>' .
        '</a>';
...
if (!function_exists('elessi_add_custom_sale_flash')) :
    function elessi_add_custom_sale_flash() {
         ...
         * Custom Badge
         */
        $badge_hot = elessi_get_custom_field_value($product->get_id(), '_bubble_hot');
        $badges .= $badge_hot ? '<div class="badge hot-label">' . $badge_hot . '</div>' : '';
        ...
        if ($product->is_on_sale()) :
            ...
            $product_type = $product->get_type();
            if ($product_type == 'variable') :
                $badges .= '<div class="badge sale-label sale-variable">' . esc_html__('SALE', 'elessi-theme') . '</div>';
                ...
                if (is_numeric($sales_price)) :
                    ...
                    $badges .= '<div class="badge sale-label">' . sprintf('%s', $maximumper . '%') . '</div>';
                endif;
            endif;
