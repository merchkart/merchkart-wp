 ...
 * @returns {undefined}
 */
function nasa_Ajax_filter($, _url, _page, _catid, _order, _variations, _hasPrice, _min, _max, _hasSearch, _s, _this, _taxonomy, _totop, loadMore, buildUrl) {
    ...
    var _top_filter = $('.nasa-top-cat-filter').length > 0 ? true : false;
...
function add_compare_product(_id, $) {
    ...
    if(
        ...
        _urlAjax = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nasa_add_compare_product');
    }
...
function remove_compare_product(_id, $) {
    ...
    if(
        ...
        _urlAjax = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nasa_remove_compare_product');
    }
...
function removeAll_compare_product($) {
    ...
    if(
        ...
        _urlAjax = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nasa_remove_all_compare');
    }
...
function changeLayoutShopPage($, _this) {
    ...
    $.cookie('gridcookie', value_cookie, {expires: 7, path: '/'});
    loadTipTop($);
    initThemeNasaGiftFeatured($);
...
function nasa_single_add_to_cart($, _this, _id, _quantity, _type, _variation_id, _variation, _data_wishlist) {
    ...
    // Ajax add to cart
        ...
        if(
            ...
            _urlAjax = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nasa_single_add_to_cart');
        }
                    ...
                    if (res.error) {
                        ...
                        if ($('.wishlist_sidebar').length > 0) {
                            if (typeof res.wishlist !== 'undefined') {
                                ...
                                setTimeout(function() {initWishlistIcons($, true);}, 350);
...
function loadComboPopup($, _this) {
    ...
    if(
        ...
        _urlAjax = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nasa_combo_products');
    }
...
function initMainMenuVertical($) {
    if($('#nasa-menu-sidebar-content .nasa-menu-for-mobile').length <= 0) {
        ...
        if($('.nasa-header-content-builder').length && $('#nasa-menu-sidebar-content').find('.nasa-nav-extra-warp').length <= 0) {
            $('#nasa-menu-sidebar-content').append('<div class="nasa-nav-extra-warp nasa-show"><div class="desktop-menu-bar"><div class="mini-icon-mobile"><a href="javascript:void(0);" class="nasa-mobile-menu_toggle bar-mobile_toggle"><span class="fa fa-bars"></span></a></div></div></div>');
        }
    }
}
    ...
    if($('.nasa-single-product-scroll').length === 1) {
        ...
        $(window).scroll(function(){
            ...
            if(!_inMobile) {
                ...
                if(typeof changeWidthSingleAddToCart !== 'undefined') {
                    clearTimeout(changeWidthSingleAddToCart);
                    changeWidthSingleAddToCart = setTimeout(function (){
                        change_width_single_add_to_cart($);
                    }, 100);
                }
            } else {
...
function loadProductsMasonryIsotope($, hasDestroy, _init_ltr) {
    if($('.nasa-products-masonry-isotope').length) {
        if(_isotope_init && hasDestroy) {
            _isotope = $('.nasa-products-masonry-isotope .products').isotope('destroy');
            _isotope_init = false;
        }
...
function initCompareIcons($) {
    if($('.nasa-compare-list-bottom .nasa-compare-item .nasa-remove-compare').length > 0) {
        var _comparetArr = [];
        $('.nasa-compare-list-bottom .nasa-compare-item .nasa-remove-compare').each(function() {
            _comparetArr.push($(this).attr('data-prod'));
        });
                ...
                if(_countSelect && _selected === _countSelect) {
                    ...
                    if(typeof _single_variations[variation.variation_id] === 'undefined') {
                        ...
                        if(
                            ...
                            _urlAjax = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nasa_get_gallery_variation');
                        }
...
function changeGalleryVariableQuickviewProduct($, _data) {
    ...
    if(typeof _lightbox_variations[_data.variation_id] === 'undefined') {
        ...
        if(
            ...
            _urlAjax = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nasa_quickview_gallery_variation');
        }
...
function reverseFillInputPlaceHolder($, _input, index) {
    ...
    var _index = typeof index !== 'undefined' ? index : _str.length;
    if (_index >= 0) {
...
function after_added_to_cart($) {
    ...
    if(
        ...
        _urlAjax = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nasa_after_add_to_cart');
    }
    ...
    if (_urlAjax) {
        $.ajax({
            ...
            beforeSend: function () {
                /*
                if ($('.nasa-after-add-to-cart-popup').length <= 0 && $('#nasa-before-load').length) {
...
function reloadMiniCart($) {
    ...
    if(
        ...
        _urlAjax = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nasa_reload_fragments');
    }
