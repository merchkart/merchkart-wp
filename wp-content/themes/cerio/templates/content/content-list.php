...
<div class="list-post">
    <article id="post-<?php esc_attr(the_ID()); ?>" <?php post_class(); ?>>
		...
		<?php elseif( $format == 'video' || $format == 'audio' ) : ?>
			...
			<div class="entry-thumb single-thumb">
				<a class="post-thumbnail" href="<?php esc_url(the_permalink()); ?>" title="<?php echo the_title_attribute(); ?>">
					<?php the_post_thumbnail( 'cerio-full-width', array( 'alt' => get_the_title() ) );	?>      
				</a> 
			</div>
			<?php } ?>
						...
						<div class="slick-carousel" data-columns4="1" data-columns3="1" data-columns2="1" data-columns1="1" data-columns="1" data-nav="true">
								<?php
									if($ids){
										...
										foreach ( $ids as $i => $id ){ ?>
											<div class="item">
													<?php echo wp_get_attachment_image($id, 'cerio-full-width'); ?>
											</div>
										<?php }	
									}
								?>
		...
		<div class="post-content">
        	...
        	<?php
        		if (cerio_get_config('blog-excerpt')) {
                    echo cerio_get_excerpt( cerio_get_config('list-blog-excerpt-length',50), false);
                }
			?>
			<div class="post-date">
				<span><?php echo cerio_time_link(); ?></span>
			</div>
    	</div>	
    </article><!-- #post-## -->
</div>