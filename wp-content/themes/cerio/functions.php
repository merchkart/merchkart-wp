...
function cerio_custom_css() {
	...
	if (!is_admin()) {
		...
		wp_add_inline_style( 'cerio-style-template', implode($custom_css) );
	}
}
add_action('wp_enqueue_scripts', 'cerio_custom_css' );
function cerio_custom_js() {
	if (!is_admin()) {
		...
		wp_add_inline_script( 'cerio-script', $custom_js );
	}
}
add_action('wp_enqueue_scripts', 'cerio_custom_js' );