<?php
	function cerio_get_config($option,$default='1'){
		...
		if(isset($params[$option]) && $params[$option]){
			...
			$value = isset($cerio_settings[$option]) ? $cerio_settings[$option] : $default;
			return $value;
		}
	}
	function cerio_get_query_string(){
		global $wp_rewrite;
	...
	function cerio_global_settings(){
		global $cerio_settings;
		return $cerio_settings;
	}
	function cerio_limit_verticalmenu(){
		global $cerio_page_id;
		$vertical = new stdClass;
		$max_number_1530	= cerio_get_config('max_number_1530',12);
		$vertical->max_number_1530 	= (get_post_meta( $cerio_page_id, 'max_number_1530', true )) ? get_post_meta($cerio_page_id, 'max_number_1530', true ) : $max_number_1530;
		
		$max_number_1200	= cerio_get_config('max_number_1200',8);
		$vertical->max_number_1200  	= (get_post_meta( $cerio_page_id, 'max_number_1200', true )) ? get_post_meta($cerio_page_id, 'max_number_1200', true ) : $max_number_1200;
		
		$max_number_991		= cerio_get_config('max_number_991',6);
		$vertical->max_number_991  	= (get_post_meta( $cerio_page_id, 'max_number_991', true )) ? get_post_meta($cerio_page_id, 'max_number_991', true ) : $max_number_991;
		
		return $vertical;
	}
	if ( ! function_exists( 'cerio_popup_newsletter' ) ) {
		function cerio_popup_newsletter() {
			$cerio_settings = cerio_global_settings(); 
			...
			echo '<div class="wp-newletter">';
				if(isset($cerio_settings['background_newletter_img']['url']) && !empty($cerio_settings['background_newletter_img']['url'])){
					echo '<div class="image"> <img src='.esc_url($cerio_settings['background_newletter_img']['url']).' alt="'.esc_attr__( 'Image Newletter','cerio' ).'"></div>';
				}
	...
	function cerio_config_font(){
		...
		foreach ($text_fonts as $text) {
			if(cerio_get_config($text))
				$config_fonts[$text] = cerio_get_config($text);
		}
		return $config_fonts;
	}
	function cerio_get_class(){
		$class = new stdClass;
		$sidebar_left_expand 		= cerio_get_config('sidebar_left_expand',3);
		$sidebar_left_expand_md 	= cerio_get_config('sidebar_left_expand_md',3);
		$class->class_sidebar_left  = 'col-xl-'.$sidebar_left_expand.' col-lg-'.$sidebar_left_expand_md.' col-md-12 col-12';
		$sidebar_right_expand 		= cerio_get_config('sidebar_right_expand',3);
		$sidebar_right_expand_md 	= cerio_get_config('sidebar_right_expand_md',3);
		$class->class_sidebar_right  = 'col-xl-'.$sidebar_right_expand.' col-lg-'.$sidebar_right_expand_md.' col-md-12 col-12';
		$sidebar_blog = cerio_blog_sidebar();
		...
		$post_single_layout = cerio_post_sidebar();
		...
		$sidebar_detail_product = cerio_get_config('sidebar_detail_product');
		...
		$blog_col_large 	= 12/(cerio_get_config('blog_col_large',3));
		$blog_col_medium = 12/(cerio_get_config('blog_col_medium',3));
		$blog_col_sm 	= 12/(cerio_get_config('blog_col_sm',3));
		$class->class_item_blog = 'col-xl-'.$blog_col_large.' col-lg-'.$blog_col_medium.' col-md-'.$blog_col_sm.' col-sm-12 col-12';
		return $class;
	}
	function cerio_post_sidebar(){
		$sidebar_post = get_post_meta( get_the_ID(), 'post_single_layout', true );
		if( !empty($sidebar_post)){
			$post_single_layout = $sidebar_post;
		}else{
			$post_single_layout = cerio_get_config('post-single-layout','left');
		}
		return 	$post_single_layout;
	}
	function cerio_blog_view(){
		...
		if( !empty($layout_blog) ){
			...
			$blog_view = cerio_get_config('layout_blog','list');
		}
		return 	$blog_view;
	}
	function cerio_blog_sidebar(){
		$category = get_category( get_query_var( 'cat' ) );
		$id_category =  ( isset($category->term_id) && $category->term_id ) ? $category->term_id : 0;
		...
		if( !empty($sidebar_blog) ){
			...
			$sidebar 		= cerio_get_config('sidebar_blog','left');
		}
		return 	$sidebar;
	}	
	function cerio_is_customize(){
		return isset($_POST['customized']) && ( isset($_POST['customize_messenger_chanel']) || isset($_POST['wp_customize']) );
	}	
	function cerio_search_form( $form ) {
		$form = '<form role="search" method="get" id="searchform" class="search-from" action="' . esc_url(home_url( '/' )) . '" >
					<div class="container">
						<div class="form-content">
							<input type="text" value="' . esc_attr(get_search_query()) . '" name="s"  class="s" placeholder="' . esc_attr__( 'Search...', 'cerio' ) . '" />
							<button id="searchsubmit" class="btn" type="submit">
								<i class="icon_search"></i>
								<span>' . esc_html__( 'Search', 'cerio' ) . '</span>
							</button>
						</div>
					</div>
				  </form>';
	...
	add_filter( 'get_search_form', 'cerio_search_form' );
	// Remove each style one by one
	add_filter( 'woocommerce_enqueue_styles', 'cerio_jk_dequeue_styles' );
	function cerio_jk_dequeue_styles( $enqueue_styles ) {
	...
	function cerio_woocommerce_breadcrumb( $args = array() ) {
	...
	add_filter('woocommerce_add_to_cart_fragments', 'cerio_woocommerce_header_add_to_cart_fragment');
	function cerio_woocommerce_header_add_to_cart_fragment( $fragments )
	{
	    global $woocommerce;
	    ob_start(); 
	...
	function cerio_display_view(){
		echo cerio_grid_list();
    }
	function cerio_grid_list(){
		$active_column_2 = $active_column_3 = $active_column_4 = $active_list = '';
		$product_col_large = cerio_get_config('product_col_large',4);
		$category_view_mode = cerio_category_view();
		$query_string = '?'.cerio_get_query_string();
	...
	function cerio_category_view(){
		...
		if( $category_view &&  $id_category != 0 ){
			...
			$category_view_mode 		= cerio_get_config('category-view-mode','grid');	
		}
		return 	$category_view_mode;
	}	
	function cerio_main_menu($id, $layout = "") {
		global $cerio_settings, $post;
		$show_cart = $show_wishlist = false;
		if ( isset($cerio_settings['show_cart']) ) {
		$show_cart            = $cerio_settings['show_cart'];
		}
		if ( isset($cerio_settings['show_wishlist']) ) {
		$show_wishlist            = $cerio_settings['show_wishlist'];
		}
		$vertical_header_text = (isset($cerio_settings['vertical_header_text']) && $cerio_settings['vertical_header_text']) ? $cerio_settings['vertical_header_text'] : '';
		$page_menu = $menu_output = $menu_full_output = $menu_with_search_output = $menu_float_output = $menu_vert_output = "";
		$main_menu_args = array(
			'echo'            => false,
			'theme_location' => 'main_navigation',
			'walker' => new cerio_mega_menu_walker,
		);
		...
		if(function_exists('wp_nav_menu')) {
			if (has_nav_menu('main_navigation')) {
				...
				if(is_user_logged_in()){
					$menu_output .= '<div class="no-menu">'. esc_html__("Please assign a menu to the Main Menu in Appearance > Menus", 'cerio').'</div>';
				}
			}
		}
		$menu_output .= '</nav>'. "\n";
	...
	add_action('admin_enqueue_scripts','cerio_upload_scripts');	
	function cerio_upload_scripts()
    {
        wp_enqueue_script('media-upload');
        wp_enqueue_script('thickbox');
        wp_enqueue_style('thickbox');
    }		
	function cerio_body_classes( $classes ) {
		if (is_single() || is_page() && !is_front_page()) {
			$classes[] = basename(get_permalink());
		}			
		$type_banner = cerio_get_config('banners_effect');
		$classes[] = $type_banner;		
	...
	add_filter( 'body_class', 'cerio_body_classes' );
	function cerio_post_classes( $classes ) {
		if ( ! post_password_required() && ! is_attachment() && has_post_thumbnail() ) {
			$classes[] = 'has-post-thumbnail';
		}
		return $classes;
	}
	add_filter( 'post_class', 'cerio_post_classes' );
	function cerio_get_excerpt($limit = 45, $more_link = true, $more_style_block = false) {
		$cerio_settings = cerio_global_settings();
		if (!$limit) {
			$limit = 45;
		}
		...
		if($content)
		{
			...
			$content = cerio_strip_tags( apply_filters( 'the_content', $content ) );
			...
			if ($more_link && $check_readmore) {
				if ($more_style_block) {
					$content .= ' <a class="read-more read-more-block" href="'.esc_url( apply_filters( 'the_permalink', get_permalink() ) ).'">'.esc_html__('Read more', 'cerio').'</a>';
				} else {
					$content .= ' <a class="read-more" href="'.esc_url( apply_filters( 'the_permalink', get_permalink() ) ).'">'.esc_html__('Read more', 'cerio').'</a>';
				}
			}
		}
		return $content;
	}
	function cerio_strip_tags( $content ) {
	...
	if( !function_exists( 'cerio_get_direction' ) ) :
	function cerio_get_direction(){
		$direction = cerio_get_config('direction','ltr');		
		if (isset($_COOKIE['cerio_direction_cookie']))
			$direction = $_COOKIE['cerio_direction_cookie'];
		if(isset($_GET['direction']) && $_GET['direction'])
			$direction = $_GET['direction'];
		return 	$direction;
	}
	endif;	
	function cerio_get_entry_content_asset( $post_id ){
	...
	function cerio_loading_overlay(){
		$cerio_settings = cerio_global_settings();
		if(isset($cerio_settings['show-loading-overlay']) && $cerio_settings['show-loading-overlay'] ){
	...
	function cerio_header_logo(){
		$cerio_settings = cerio_global_settings(); 
		$sitelogo = (isset($cerio_settings['sitelogo']['url']) && $cerio_settings['sitelogo']['url']) ? $cerio_settings['sitelogo']['url'] : "";
	...
	<?php }
	function cerio_top_menu(){
		$cerio_settings = cerio_global_settings();
		echo '<div class="wpbingo-menu-wrapper">
			<div class="megamenu">
				<nav class="navbar-default">
					<div  class="bwp-navigation primary-navigation navbar-mega" data-text_close = "'.esc_html__('Close','cerio').'">
						'.cerio_main_menu( 'main-navigation', 'float' ).'
					</div>
				</nav> 
			</div>       
		</div>';
	}	
	function cerio_top_menu_right(){
		$cerio_settings = cerio_global_settings();
		echo '<div class="wpbingo-menu-wrapper">
			<div class="megamenu">
	...
	function cerio_navbar_vertical_menu(){
		echo '<div class="wpbingo-verticalmenu-mobile">
			<div class="navbar-header">
				<button type="button" id="show-verticalmenu"  class="navbar-toggle">
					<span>'. esc_html__("Vertical","cerio") .'</span>
				</button>
			</div>
		</div>';
	}
	
	function cerio_vertical_menu() {
		global $cerio_settings;
		$menu_output = "";
		...
		if(function_exists('wp_nav_menu')) {
			if (has_nav_menu('vertical_menu')) {
				$menu_output .=	'<h3 class="widget-title"><i class="fa fa-bars" aria-hidden="true"></i>'.esc_html__('All Departments','cerio').'</h3>';
	...
	function cerio_dropdown_vertical_menu(){
		global $cerio_page_id;
		$show_vertical_menu  = (get_post_meta( $cerio_page_id, 'show_vertical_menu', true )) ? get_post_meta($cerio_page_id, 'show_vertical_menu', true ) : 'accordion';
		return $show_vertical_menu;
	}	
	
	function cerio_category_post(){
		global $post;
	...
	function cerio_copyright(){
		$cerio_settings = cerio_global_settings();?>
		<div class="bwp-copyright">
			<div class="container">		
			    <div class="row">
					<?php if(isset($cerio_settings['footer-copyright']) && $cerio_settings['footer-copyright']) : ?>		
						<div class="site-info col-sm-6 col-xs-12">
							<?php echo esc_html($cerio_settings['footer-copyright']); ?>
						</div><!-- .site-info -->
					<?php else: ?>					
						<div class="site-info col-sm-6 col-xs-12">
							<?php echo esc_html__( 'Copyright 2020 ','cerio'); ?>					
							<a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html__('cerio', 'cerio'); ?></a>
							<?php echo esc_html__( '. All Rights Reserved.','cerio'); ?>
						</div><!-- .site-info -->		
					<?php endif; ?>
					<?php if(isset($cerio_settings['footer-payments']) && $cerio_settings['footer-payments']) : ?>
						<div class="payment col-sm-6 col-xs-12">
							<a href="<?php echo isset($cerio_settings['footer-payments-link']) ? esc_url($cerio_settings['footer-payments-link']) : "#"; ?>">
								<img src="<?php echo isset($cerio_settings['footer-payments-image']['url']) ? esc_url($cerio_settings['footer-payments-image']['url']) : ""; ?>" alt="<?php echo isset($cerio_settings['footer-payments-image-alt']) ? esc_attr($cerio_settings['footer-payments-image-alt']) : ""; ?>" />
							</a>
						</div>		
					<?php endif; ?>	
				</div>
	...
	function cerio_render_footer($footer_style){
		$elementor_instance = Elementor\Plugin::instance();
		return $elementor_instance->frontend->get_builder_content_for_display( $footer_style );
	}
	if( !is_admin() ){
		add_filter( 'language_attributes', 'cerio_direction', 20 );
		function cerio_direction( $doctype = 'html' ){
	   		$direction = cerio_get_direction();
	   		...
	   		$cerio_output = implode(' ', $attribute);
	   		return $cerio_output;
		}
	}	
	function cerio_comment( $comment, $args, $depth ) {
		if ( 'div' == $args['style'] ) {
			$tag = 'div';
			$add_below = 'comment';
		} else {
		...
		<div class="media">
			...
			<div class="media-body">
				<div class="comment-meta media-content commentmetadata">
					...
					<?php printf( wp_kses_post( '<h2 class="media-heading">%s</h2>', 'cerio' ), get_comment_author_link() ); ?>
					</div>
					<?php if ( '0' == $comment->comment_approved ) : ?>
					<em class="comment-awaiting-moderation"><?php echo esc_html__( 'Your comment is awaiting moderation.', 'cerio' ); ?></em>
					<?php endif; ?>
					<div class="media-silver">
						<a href="<?php echo esc_url( htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ); ?>" class="comment-date">
							<?php echo '<time datetime="' . get_comment_date( 'c' ) . '">' . get_comment_date() . '</time>'; ?>
						</a>
						<?php edit_comment_link( __( 'Edit', 'cerio' ), '  ', '' ); ?>
					</div>
	...
	function cerio_prefix_kses_allowed_html($allowed_tags, $context) {
		switch($context) {
			case 'social': 
			$allowed_tags = array(
				'a' => array(
	...
	add_filter( 'wp_kses_allowed_html', 'cerio_prefix_kses_allowed_html', 10, 2);
	if ( ! function_exists( 'wp_body_open' ) ) {
		function wp_body_open() {
			do_action( 'wp_body_open' );
		}
	}
	function cerio_menu_mobile($vertical = false){
	$cerio_settings = cerio_global_settings();
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-xl-4 col-lg-4 col-md-4 col-sm-3 col-3 header-left">
					<div class="navbar-header">
						<button type="button" id="show-megamenu"  class="navbar-toggle">
							<span><?php echo esc_html__("Menu","cerio"); ?></span>
						</button>
					</div>
				</div>
				...
				<div class="col-xl-4 col-lg-4 col-md-4 col-sm-3 col-3 header-right">
					<?php if($vertical){?>
						<?php cerio_navbar_vertical_menu(); ?>
					<?php } ?>
				...
				<div class="box-content">
					<div class="form-login active">
						<form method="post" class="login">
							<h2><?php echo esc_html__("Sign in","cerio") ?></h2>
							<div class="content">
								<?php do_action( 'woocommerce_login_form_start' ); ?>
								<div class="username">
									<input type="text" required="required" class="input-text" name="username" id="username" placeholder="<?php echo esc_attr__("Your name","cerio") ?>" />
								</div>
								<div class="password">
									<input class="input-text" required="required" type="password" name="password" id="password" placeholder="<?php echo esc_attr__("Password","cerio") ?>" />
								</div>
								<div class="rememberme-lost">
									<div class="rememberme">
										<input name="rememberme" type="checkbox" id="rememberme" value="forever" />
										<label for="rememberme" class="inline"><?php echo esc_html__( 'Remember me', 'cerio' ); ?></label>
									</div>
									<div class="lost_password">
										<a href="<?php echo esc_url( wc_lostpassword_url() ); ?>"><?php echo esc_html__( 'Lost your password?', 'cerio' ); ?></a>
									</div>
								</div>
								<div class="button-login">
									<?php wp_nonce_field( 'woocommerce-login', 'woocommerce-login-nonce' ); ?>
									<input type="submit" class="button" name="login" value="<?php echo esc_html__( 'Login', 'cerio' ); ?>" /> 
								</div>
								<a class="button-next-reregister" href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>"><?php echo esc_html__("Create An Account","cerio") ?></a>
							</div>
						</form>
					</div>
				</div>
